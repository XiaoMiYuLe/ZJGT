...
if (fs.existsSync(path.join(__dirname, '../../../404.html'))) {
    path_404 = path.join(__dirname, '../../../404.html');
} else {
    path_404 = path.join(__dirname, '../template/404.html');
}
const DEFAULT_404_TEMPLATE = path_404;
...
module.exports = class Tracer {
    ...
    getTemplateContent() {
        ...
        const readFilesAsync = Object.keys(this.templatesPath).map(status =>
            readFileAsync(this.templatesPath[status], {
                encoding: 'utf-8'
            })
            .catch(() =>
                readFileAsync(status !== '404' ? DEFAULT_500_TEMPLATE : DEFAULT_404_TEMPLATE, {
                    encoding: 'utf-8'
                })
            ).then(template => {
