            ...
            return this.each(function () {
                if (!('placeholder' in document.createElement('input'))) {
                    var input = $(this),
                        ...
                        inputPaddingLeft = parseInt(input.css("padding-left"), 10);
...
$(function () {
    ...
    $(".side-menu dl dt").on("click", function () {
        var $btn = $(this);
        var $wrap = $btn.closest("dl");
        var $item = $btn.siblings("dd");
        if ($item.length != 0 && !$wrap.hasClass("cur")) {
            $wrap.toggleClass("selected");
            $item.toggle();
        }
    });

    $(".bank-area").on("click", ".select-other", function () {
        $(this).closest(".bank-area").find(".bank-list li").removeClass("hide");
        $(this).remove();
    });
});
